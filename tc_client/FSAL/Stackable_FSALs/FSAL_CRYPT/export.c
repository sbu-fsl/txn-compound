...
{
	struct cryptfs_fsal_export *myself;
	...
	myself = container_of(exp_hdl, struct cryptfs_fsal_export, export);
...
/* cryptfs_export_ops_init
 * overwrite vector entries with the methods that we support
 */

void cryptfs_export_ops_init(struct export_ops *ops)
{
	ops->release = release;
	ops->lookup_path = cryptfs_lookup_path;
	ops->extract_handle = extract_handle;
	ops->create_handle = cryptfs_create_handle;
				   ...
				   struct fsal_export **export)
{
	...
	if (fsal_stack == NULL) {
		LogMajor(COMPONENT_FSAL,
			 "cryptfs_create_export: failed to lookup for FSAL %s",
			 fs_specific);
	...
	if (FSAL_IS_ERROR(expres)) {
		LogMajor(COMPONENT_FSAL,
			 "cryptfs_create_export: failed to call create_export on underlying FSAL %s",
			 fs_specific);
		return expres;
	}
	...
	cryptfs_export_ops_init((*export)->ops);
	cryptfs_handle_ops_init((*export)->obj_ops);
