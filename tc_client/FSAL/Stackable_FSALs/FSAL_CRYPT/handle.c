...
{
	return next_ops.obj_ops->create(dir_hdl, opctx, name, attrib, handle);
}
...
static fsal_status_t makenode(struct fsal_obj_handle *dir_hdl, const struct req_op_context *opctx, const char *name, object_file_type_t nodetype,	/* IN */
			      ...
			      struct fsal_obj_handle **handle)
{
	return next_ops.obj_ops->mknode(dir_hdl, opctx, name, nodetype, dev,
					attrib, handle);
}
...
static fsal_status_t makesymlink(struct fsal_obj_handle *dir_hdl,
				 ...
				 struct fsal_obj_handle **handle)
{
	return next_ops.obj_ops->symlink(dir_hdl, opctx, name, link_path,
					 attrib, handle);
}
...
static fsal_status_t readsymlink(struct fsal_obj_handle *obj_hdl,
				 ...
				 bool refresh)
{
	return next_ops.obj_ops->readlink(obj_hdl, opctx, link_content,
					  refresh);
}
...
static fsal_status_t linkfile(struct fsal_obj_handle *obj_hdl,
			      ...
			      const char *name)
{
	return next_ops.obj_ops->link(obj_hdl, opctx, destdir_hdl, name);
}
...
static fsal_status_t renamefile(struct fsal_obj_handle *olddir_hdl,
				...
				const char *new_name)
{
	return next_ops.obj_ops->rename(olddir_hdl, opctx, old_name, newdir_hdl,
					new_name);
}
...
static fsal_status_t getattrs(struct fsal_obj_handle *obj_hdl,
			      const struct req_op_context *opctx)
{
	return next_ops.obj_ops->getattrs(obj_hdl, opctx);
}
...
{
	return next_ops.obj_ops->setattrs(obj_hdl, opctx, attrs);
}
...
static fsal_status_t file_unlink(struct fsal_obj_handle *dir_hdl,
				 ...
				 const char *name)
{
	return next_ops.obj_ops->unlink(dir_hdl, opctx, name);
}
...
void cryptfs_handle_ops_init(struct fsal_obj_ops *ops)
{
	ops->release = release;
	ops->lookup = lookup;
	...
	ops->open = cryptfs_open;
	ops->status = cryptfs_status;
	ops->read = cryptfs_read;
	ops->write = cryptfs_write;
	ops->commit = cryptfs_commit;
	ops->lock_op = cryptfs_lock_op;
	ops->close = cryptfs_close;
	ops->lru_cleanup = cryptfs_lru_cleanup;
	ops->handle_digest = handle_digest;
	ops->handle_to_key = handle_to_key;

	/* xattr related functions */
	ops->list_ext_attrs = cryptfs_list_ext_attrs;
	ops->getextattr_id_by_name = cryptfs_getextattr_id_by_name;
	ops->getextattr_value_by_name = cryptfs_getextattr_value_by_name;
	ops->getextattr_value_by_id = cryptfs_getextattr_value_by_id;
	ops->setextattr_value = cryptfs_setextattr_value;
	ops->setextattr_value_by_id = cryptfs_setextattr_value_by_id;
	ops->getextattr_attrs = cryptfs_getextattr_attrs;
	ops->remove_extattr_by_id = cryptfs_remove_extattr_by_id;
	ops->remove_extattr_by_name = cryptfs_remove_extattr_by_name;
