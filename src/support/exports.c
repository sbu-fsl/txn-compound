 ...
 *                        not already loaded)
 * @param[out] fsal_hdl_p FSAL module pointer or NULL if not found
		   ...
		   struct config_error_type *err_type)
{
	...
	*fsal_hdl_p = lookup_fsal(name);
	if (*fsal_hdl_p == NULL) {
		int retval;
		config_file_t myconfig;

		retval = load_fsal(name, fsal_hdl_p);
		if (retval != 0) {
			LogCrit(COMPONENT_CONFIG,
		...
		status = (*fsal_hdl_p)->ops->init_config(*fsal_hdl_p, myconfig);
		if (FSAL_IS_ERROR(status)) {
			LogCrit(COMPONENT_CONFIG,
