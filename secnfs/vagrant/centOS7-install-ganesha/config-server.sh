...
sudo yum install -y gcc cmake git libtool wget bison flex bc
...
if [ ! -d fsl-nfs-ganesha ]; then
  yes | git clone -b integrity-2.1 $git_repo;
fi
...
# Apply patches
for i in ~/fsl-nfs-ganesha/nfs4int/pi-patchset-v2/?-*.patch;
	do
...
# Apply fixes
for i in ~/fsl-nfs-ganesha/nfs4int/pi-patchset-v2/000?-fix*.patch;
	do
		sudo patch -p1 < $i;
	done
